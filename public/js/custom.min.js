...
function deleteUser(){
    ...
    var tr={};
    /**
     * Test tout les utilisteurs chechées et renvoye leur id
     */
    if(testCheckbox(id,tr)){
        $( "#dialog-confirm" ).dialog({
            ...
            buttons: {
                "Oui": function() {
                    ...
                    /**
                     * Remove les lignes de la table des utilisateurs qui ont été supprimés.
                     */
...
function toggleForm(){
    $('div .success').remove();
    $('div .form-hidden').removeClass('form-hidden').addClass('row');
}
...
function modifyModal(id){
    data = {'id' : id, 'idCompany' : $('.right_col').attr('id'), note: $('#note'+id).val()};
        $.ajax({
        method: 'POST',
        url: '../to-do?rq=getNote',
        data: data
    }).done(function (data) {
    $('#myModal'+data.id).modal('hide');
    $('#myModal'+data.id).html('   <div class="modal-dialog">\n' +
        '                                        <!-- Modal content-->\n' +
        '                                        <div class="modal-content">\n' +
        '                                            <div class="modal-header">\n' +
        '                                                <button type="button" class="close" data-dismiss="modal">&times;</button>\n' +
        '                                                <h4 class="modal-title">Modifier une note</h4>\n' +
        '                                            </div>\n' +
        '                                            <div class="modal-body" id="modal-body'+data.id+'"">\n' +
        '                                                <textarea class="autoExpand form-control" rows="5" id="note'+data.id+'">'+data.note+'</textarea>\n' +
        '                                            </div>\n' +
        '                                            <div class="modal-footer">\n' +
        '                                                <button type="button" id="modalSend'+data.id+'" onclick="setModalNote('+data.id+')" class="btn btn-primary">Modifier</button>\n' +
        '                                                <button type="button"  class="btn btn-default close-modal" data-dismiss="modal" onclick="">Fermer</button>\n' +
        '                                            </div>\n' +
        '                                        </div>\n' +
        '                                    </div>');
