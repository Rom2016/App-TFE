...
$('#create-ext-audit').click(function() {

    data = {
        'audit-name': $('#int-audit-name').val(),
        'customer': $('#int-customer').val(),
        'responsable': $('#int-customer-responsable').val()
    };
    $.ajax({
        url: '../créer-audit-interne',
        type: 'POST',
        data: data,
...
/**
 * Envoye les fichiers images de la dropbox
 * @param testID
 * @param auditNumber
 */
...
function uploadFile(testID,auditNumber){
    var myDropzone = Dropzone.forElement("#myDropzone"+testID);
    myDropzone.on("sending", function(file, xhr, formData) {
        formData.append("id", testID);
        formData.append("auditNumber", auditNumber );
    })
...
function swalModifyProfilPic(){
    swal({
        showConfirmButton: false,
        title: 'Modifier la photo de profile',
        html: ' <div id="myDropzone" class="myDropzone dropzone">\n' +
        '\n' +
        '                                                                                                    <div class="dz-message needsclick">\n' +
        '                                                                                                        <p style="color: darkgrey;"><i>Déposer une image</i></p>\n' +
        '                                                                                                    </div>\n' +
        '                                                                                            </div>'
    })
    ...
    swalWithBootstrapButtons({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                $.ajax({
                    url: '../administration/supprimer-utilisateur',
                    type: 'POST',
                    data: 'id='+id,
                    dataType: 'json'
                })
...
function addUser(isSuperAdmin){


    role = getRole(isSuperAdmin);
    const swalWithBootstrapButtons = swal.mixin({
        confirmButtonClass: 'btn btn-success',
        cancelButtonClass: 'btn btn-danger',
        buttonsStyling: false,
    ...
    swalWithBootstrapButtons({
        ...
        html: ' <form action="../administration" method="post">\n' +
        '                                        <div class="row">\n' +
        '                                            <div class="col-md-6">\n' +
        ...
        '                                    <button type="submit" id="submit_new_user" class="btn btn-success" name="submit" value="newUser"><i class="fa fa-check"></i></button></form>',
        showConfirmButton: false,
        cancelButtonText: '<i class="fa fa-close"></i>',
        showLoaderOnConfirm: true
    });
...
function getRole(isSuperAdmin){
    ...
    roleSelect = '<select id="selectRole" name="selectRole" style="padding: 10px; border: none; border-bottom: solid 1px lightgrey;">';

    $.each(roles, function (key, value) {
        if (value != 'ROLE_SUPER_ADMIN') {
            roleSelect = roleSelect + '<option style="color: ;">' + value + '</option>'
        }
        else if(value == 'ROLE_SUPER_ADMIN' && isSuperAdmin)
        {
            roleSelect = roleSelect + '<option style="color: ;">' + value + '</option>'
        }
    })
...
function emailAutoCompletion(){
    $('#userEmail').val($('#fName').val()+'.'+$('#sName').val()+'@anderson.be');
}

function saveRole(role,id){
    $.ajax({
        url: '../administration/changer-role',
        type: 'POST',
        ...
        .done(function(response){
            ...
            if(response == 'ROLE_USER'){
                ...
                color = 'alert-warning'
            }else if(response == 'ROLE_SUPER_ADMIN'){
                color = 'alert-danger'
            }
            $('#userRole'+id).html('<span class="alert '+color+' ">'+response+'</span>\n');
        })
        .fail(function(){
            swal('Oops...', 'Un problème est survenu, réessayez plus tard!', 'error');
        });
}
function changeRole(id,isSuperAdmin){
    var roles = {};
    $.ajax({
        url: '../administration/roles',
        type: 'POST',
    ...
    roleSelect = '<select id="selectRole" name="selectRole" style="padding: 5px; background-color: white" onchange="saveRole(this.value,'+id+')">' +
        '    <option selected disabled>--Sélectionner un role--</option>\n';
...
function dialogConfirm(id) {
    $('#dialog-confirm'+id).dialog({
        ...
        buttons: {
            "Confirmer": function() {
                var form = new FormData($('#form'+id)[0]);
                link = '../selection-test',
                    $.ajax({
                        method: 'POST',
                        url: link,
...
/**
 * Envoye les id des utilisateurs à supprimer depuis la console d'administration
 *
 */

/**
 * Fonction qui s'occupe d'aller chercher les solutions des tests sélectionnés dans le résultat de l'audit et les ramener dans le récap
 */
$('.check-audit').click(function() {
    ...
    if(this.checked){
        data = {};
        /**
         * L'id du test et la taille de l'entreprise envoyée au  serveur
         * @type {{id: *|jQuery, size: *|jQuery}}
         */
        data = {'id': $(this).attr('value'), 'size': $('#size-company').attr('value')};
        /**
         * Il suffit d'utiliser la route défini dans le contrôleur pour exécuter la bonne méthode
         * @type {string}
         */
        link = '../solution-test';
        $.ajax({
            ...
            $('#container-solution').append(response);
            $(".accordion").accordion({
                collapsible: true,
                autoHeight: true,
                heightStyle: "content",
...
/**
 * Fonction utilisée pour supprimer des tests d'une phase existante
 * @param id
 * @param t
 */
function deleteTest(id,t){
    j = {'id': id};
        $.ajax({
            method: 'POST',
            url: '../supprimer-test-phase',
            data: j
        }).done(function () {
            $('#'+id).closest(t).remove();
...
/**
 * fonction qui vérifie si les checkbox cochées dans une table et de récupérer l'id de l'élément qui l'est.
 * @param id
 * @param tr
 * @returns {boolean}
 */
function testCheckbox(id,tr){
    var i = 0;
...
/**
 * Fonction qui va chercher tous les tests liés à un test d'infra dont la réponse est oui et l'ajoute dynamiquement au contenu de l'audit.
 *
 * @param testId
 */
function  getInfraTest(testId) {
    if ($('#radioPos' + testId).is(':checked')) {
        id = {}
        id = {'testId': testId, 'rq': 'add-groups'};
        $.ajax({
            method: 'POST',
            url: '../créer-audit',
            data: id
            ...
            $.each(response, function (key, value) {
                $.each(value, function (k, v) {
                    /**
                     * Boucle sur le JSON avec tout les tests trouvés et append au bon endroit dans l'audit((l'onglet de la bonne phase)
                     */
                    $('#phase'+key).append(v).fadeIn('slow');
                    $('.check-audit').iCheck({
...
/**
 * Fonction inverse à celle ci-dessus et qui remove tous les tests liés à l'infrastructure quand l'utilisateur clique sur Non
 * @param testId
 */
function removeInfraTest(testId){
    id = {};
    id = {'testId': testId};
    $.ajax({
        method: 'POST',
        url: '../créer-audit?rq=remove-infra-groups',
        data: id
    }).done(function (response) {
        $.each(response, function (k, v) {
            $('#group-form' + v.test_phase.id).remove();
        })
    })
}

/**
 * Fonction qui met en surbrillance les lignes d'une table cochées par une checkbox
 * @param box
 */
...
/**
 * Fonction qui gère l'activation et la désactivation des groupes d'enfant suivant la réponse au test parent
 * @param id
 */
function toggleClass(id) {
     ...
     * Si parent checked
     */
    $('#checkbox-audit'+id).on('ifChecked', function (){
        $('#group-form'+id).removeClass('group-form-unchecked');
        $('#group-form'+id).addClass('group-form-checked');


        $('#group-child' + id).removeClass('group-child');
        $('#group-child' + id).addClass('toggle-group');
    })
    /**
     * Si parent unchecked
     */
    $('#checkbox-audit'+id).on('ifUnchecked', function (){
        $('#group-form'+id).removeClass('group-form-checked');
        $('#group-form'+id).addClass('group-form-unchecked');

        $('#group-child'+id+' input:checkbox').each(function(){
            if(this.checked){
        ...
        $('#group-child' + id).removeClass('toggle-group');
        $('#group-child' + id).addClass('group-child');
    ...
    $.map(data,function(val,i) {
        switch (i) {
            case 'loginError':
                ...
                break;
            case 'content':
                $('#form').removeClass('row').addClass('form-hidden');
                div = $("<div class='success'>").html(val);
                $("#form").after(div)
                break;
            case 'testAdd':
                $('#form').removeClass('row').addClass('form-hidden');
...
function saveAudit(link){
    if(!$('#phone-company').val() || !$('#name-company').val() || !$('#email-company').val()){
        $('#missing-info').remove();
        $('#wizard').after('<span class="label label-danger" id="missing-info">Veuillez compléter les informations de la société</span>');
        $('#trigger-step1').trigger( "click" );

        $('#missing-info').delay(1000).fadeOut('slow');
        return false;
    }
    ...
    content['name'] = $('#name-company').val();
    content['phone'] = $('#phone-company').val();
    content['email'] = $('#email-company').val();
...
function modalNote(id){
    ...
    data = {'id' : id, 'idCompany' : $('.right_col').attr('id'), note: $('#note'+id).val()};
    $.ajax({
        method: 'POST',
        url: '../to-do?rq=saveNote',
        data: data
    }).done(function (data) {
        $('#modal-body'+data.id).html('<span class="glyphicon glyphicon-ok" style="color: #1ABB9C"></span><span><b>Note enregistrée</b></span>');
        $('#modalSend'+data.id).remove();
