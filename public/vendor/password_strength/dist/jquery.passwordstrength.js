(function ($) {
    $.fn.passwordStrength = function (options) {
        ...
        $(progressHtml).insertAfter($('#initial-password'));
        ...
        $(this).keyup(function (event) {
            ...
            if (password.length == 0) {
                ...
                if (score < 50) {
                    $('#password-score').html('<div class="alert alert-danger">Mot de passe faible <span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span></div>');
                    $('#password-recommendation').html('<ul><li>Le mot doit contenir au moins 8 caractères</li><li>Une majuscule</li><li>Au moins un chiffre</li><li>Au moins un caractère spécial</li></ul>');
                    $('#password-progress').removeClass();
                    $('#password-progress').addClass('progress-bar progress-bar-danger');
                }
                else if (score <= 60) {
                    $('#password-score').html('<div class="alert alert-warning">Mot de passe moyen <span class="glyphicon glyphicon-ok" aria-hidden="true"></span></div>');
                    $('#password-recommendation').html('<div id="password-recommendation-heading">Pour plus de sécurité :</div><ul><li>Utiliser des majuscules et de minuscules</li><li>Utiliser au moins un nombre</li><li>Utiliser au moins un caractère spécial</li></ul>');
                    $('#password-recommendation-heading').css('text-align', 'center');
                    $('#password-progress').removeClass();
                    $('#password-progress').addClass('progress-bar progress-bar-warning');
                }
                else if (score <= 80) {
                    $('#password-score').html('<div class="alert alert-success">Mot de passe fort <span class="glyphicon glyphicon-ok" aria-hidden="true"></span></div>');
                    $('#password-recommendation-heading').css('text-align', 'center');
                    $('#password-progress').removeClass();
